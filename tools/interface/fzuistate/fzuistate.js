...
//    (This way of doing it keeps things contained in this module, see 202602101947.)
function logtimeClick() { window.open('/cgi-bin/fzlogtime.cgi?source=nonlocal&cgivar=wrap','_blank'); }
function systemClick() { window.open('/system.html', '_blank'); }
function calschedClick() { window.open('/cgi-bin/schedule-cgi.py?c=true&num_days=7&s=20','_blank'); }
function behtoolsClick() { window.open('/formalizer/system-help.html','_blank'); }
function supclearClick() { window.open('/cgi-bin/fzgraph-cgi.py?action=generic&q=true&L=delete&l=superiors&E=STDOUT&W=STDOUT','_blank'); }
function searchgraphClick() { window.open('/formalizer/fzgraphsearch-form.html','_blank'); }
function logentryClick() { window.open('/formalizer/logentry-form_fullpage.template.html','_blank'); }
function addnodeClick() { window.open('/cgi-bin/fzgraphhtml-cgi.py?edit=new','_blank'); }
function darkmodeClick() { switch_light_or_dark(); }
...
const bottombar_data = [
    [ 'logtime', logtimeClick, 'Log Time', getSelected ],
    [ 'system', systemClick, 'System', null ],
    [ 'calsched', calschedClick, 'Calendar Schedule', null ],
    [ 'behtools', behtoolsClick, 'Behavior Tools', null ],
    [ 'supclear', supclearClick, 'Clear SupNNL', getSuperiors ],
    [ 'searchgraph', searchgraphClick, 'Search Graph', null ],
    [ 'logentry', logentryClick, 'Add Log Entry', null ],
    [ 'addnode', addnodeClick, 'Add Node', null ],
    [ 'darkmode', darkmodeClick, 'Light / Dark', null ],
];
// const bottombar_data = [
...
for (let i = 0; i < bottombar_data.length; ++i) {
    if (!bottombar_data[i][3]) {
        ...
        btn.addEventListener( 'click', () => { bottombar_data[i][1](); });
...
// -- Clock Mode and Timezone Section --

var numclockmodes = 2;
var clockmode = 0;
var timezone_offset_hours = 9;

// -- Timezone input overlay
const timezone_input = document.createElement('div');
timezone_input.id = 'timezone_input';
timezone_input.innerHTML = `<div id="tz_inlay"><div id="tz_close">X</div> Time zone offset hours: <input id="tzhours" type="number" name="tzhours" value=${timezone_offset_hours}></div>`;
document.body.append(timezone_input);
...
function hide_timezone_input() {
    timezone_input.style.display = 'none';
}
function show_timezone_input() {
    console.log('Showing time zone input.');
    //tzhours.value = timezone_offset_hours;
    timezone_input.style.display = 'block';
}
...
document.getElementById('tzhours').addEventListener( 'change', function(e) {
    update_timezone_offset(parseInt(e.target.value));
    hide_timezone_input();
});
document.getElementById('tz_close').addEventListener( 'click', function() {
    hide_timezone_input();
});

const clockmodebutton = document.getElementById('clock'); // From HTML page and clock.js.
