...
fzloghtml::fzloghtml() : formalizer_standard_program(false), config(*this), flowcontrol(flow_log_interval), ga(*this, add_option_args, add_usage_top),
    ...
    usage_tail.push_back(
        ...
        "    stamp.\n"
        "7.  If '-c <num>' is specified without other time indicators, then the most\n"
        "    recent (i.e. latest) <num> Log chunks for thate Node are retruned.\n"
        "8.  An example of a custom template is:\n"
        "      'STR:{{ t_chunkopen }} {{ t_diff_mins }} {{ node_id }}\\n'\n"
        "9.  The 'json' format is available only with the '-i' interpret for day review\n"
        "    option.\n"
        "10. Automatically translated components of URLs, such as @FZSERVER@ are\n"
        "    recognized only within properly formatted HREF tags, not in text.\n"
        "11. The @FZSERVER@ tag, when used within a URL in a HREF tag is replaced\n"
        ...
        "12. If the FILE: tag is encountered wiht the '-x' option then the RegEx\n"
